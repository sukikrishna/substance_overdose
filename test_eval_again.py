...
This version aligns model architectures with run_experiments.py and focuses on:
- Single random seed (42) for reproducibility
- 30 trials per model for robust statistics
- Experiments 1 and 2 only
- Comprehensive metrics and visualizations
- Data export for later figure editing
...
# Configuration
RESULTS_DIR = 'streamlined_eval_results_fixed'
DATA_PATH = 'data_updated/state_month_overdose_2015_2023.xlsx'
TRIALS_PER_MODEL = 30

# Optimal hyperparameters aligned with run_experiments.py
OPTIMAL_PARAMS = {
...
class FixedEvaluationPipeline:
    """Fixed evaluation pipeline with aligned model architectures"""
    ...
    def build_seq2seq_model(self, look_back: int, units: int = 50):
        """Build Seq2Seq model - exactly matching run_experiments.py"""
        # Encoder
    ...
    def build_transformer_model(self, look_back: int, d_model: int = 64, num_heads: int = 4):
        """Build Transformer model - exactly matching run_experiments.py"""
    ...
    def generate_forecast(self, model, initial_sequence, num_predictions, look_back, model_type='lstm'):
        """Generate forecasts - exactly matching run_experiments.py"""
        predictions = []
    ...
    def train_sequence_model(self, model_name: str, train_data: pd.DataFrame, params: Dict):
        ...
        if model_name == 'lstm':
            ...
            model = self.build_seq2seq_model(look_back, params['units'])
        elif model_name == 'transformer':
            model = self.build_transformer_model(look_back, params['d_model'], params['num_heads'])
    ...
    def generate_predictions(self, model, model_type: str, train_data: pd.DataFrame, 
                           test_data: pd.DataFrame, params: Dict):
        ...
        if model_type == 'sarima':
            ...
            train_X, train_Y = self.create_dataset(pd.Series(train_deaths), look_back)
            train_X = train_X.reshape((train_X.shape[0], look_back, 1))
            
            if model_type == 'seq2seq':
                decoder_input = np.zeros((train_X.shape[0], 1, 1))
                train_pred = model.predict([train_X, decoder_input], verbose=0)
                train_pred = train_pred.reshape(-1)
            else:
                train_pred = model.predict(train_X, verbose=0).reshape(-1)
            
            # Test predictions (out-of-sample, autoregressive)
            initial_sequence = train_X[-1].reshape((1, look_back, 1))
            test_pred = self.generate_forecast(model, initial_sequence, 
                                             len(test_deaths), look_back, model_type)
    ...
    def run_single_trial(self, model_name: str, train_data: pd.DataFrame, 
        ...
        try:
            if model_name == 'sarima':
                ...
                # Sequence models
                model = self.train_sequence_model(model_name, train_data, params)
                train_true, train_pred, test_true, test_pred = self.generate_predictions(
                    model, model_name, train_data, test_data, params)
    ...
    def create_model_comparison_plots(self, results: Dict, train_data: pd.DataFrame, test_data: pd.DataFrame):
        ...
        # Plot actual data
        all_train_dates = train_data['Date'].values
        all_test_dates = test_data['Date'].values
        all_dates = np.concatenate([all_train_dates, all_test_dates])
        all_actual = np.concatenate([train_data['Deaths'].values, test_data['Deaths'].values])
        ...
        for model_name, model_results in results.items():
            ...
            # Handle date alignment for sequence models
            if model_name != 'sarima':
            ...
            all_model_dates = np.concatenate([train_dates, test_data['Date'].values])
            all_model_pred = np.concatenate([avg_train_pred, avg_test_pred])
    ...
    def create_individual_comparison_plots(self, results: Dict, train_data: pd.DataFrame, test_data: pd.DataFrame):
        ...
        # SARIMA metrics for intervals
        sarima_train_metrics = self.calculate_comprehensive_metrics(sarima_train_true, sarima_train_pred)
    ...
    def create_horizon_comparison_plots(self, horizon_results: Dict):
        """Create comparison plots for each forecast horizon"""
