...
for root, dirs, files in os.walk(base_dir):
    for file in files:
        if file.endswith(".csv"):
            ...
            file_name = file
            trial_num = int(file_name.split("month")[0].split("_")[1])
            batch_size = int(file_name.split("_")[1])
            loss_type = folder_name.split("_")[3]
            epochs = int(folder_name.split("_")[-1])
            lookback = int(file_name.split("month")[0].split("_")[2])
