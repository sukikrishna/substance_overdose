...
for i in range(100):
    seed = np.random.randint(0, 2**32 - 1) #ValueError: Seed must be between 0 and 2**32 - 1
    # seed = 42
    np.random.seed(seed)
    trial_num = i
    ...
    for look_back in look_backs:
        ...
        model.add(Dense(1))
        model.compile(loss=ls, optimizer='adam')
        model.fit(trainX, trainY, epochs=num_epochs, batch_size=bs, verbose=1)
        ...
        val_df = df.copy()
        val_df = val_df[val_df['Month'] <= '2019-12-01']
    ...
    val_df.to_csv(f'../tables_seeds/various_100/trial_{trial_num}_{look_back}month_predictionresults_batch_{bs}_seed_{seed}_epochs_{num_epochs}.csv')

# batch_{bs}_loss_mse_epoch_{num_epochs}