...
SEEDS = [42]
TRIALS_PER_MODEL = 30  # Number of trials per model
...
def main():
    ...
    models_to_evaluate = ['transformer']
    ...
    for model_name in models_to_evaluate:
        ...
        for trial in range(TRIALS_PER_MODEL):
            ...
            print(f"  Trial {trial + 1}/{TRIALS_PER_MODEL} (seed: {seed})")
            ...
            try:
                if model_name == 'sarima':
                    ...
                    y_true, y_pred = run_seq2seq(train_val_data['Deaths'].values, 
                                               ...
                                               use_attention=False)  # Set to False based on your table
                ...
                trial_df.to_csv(os.path.join(model_dir, f'trial_{trial + 1}_predictions.csv'), index=False)
        ...
        if model_results:
            ...
            results_df.to_csv(os.path.join(model_dir, 'all_trials_metrics.csv'), index=False)
            
            # Calculate and save summary statistics
            summary_stats = results_df[['RMSE', 'MAE', 'MAPE']].agg(['mean', 'std', 'min', 'max'])
            summary_stats.to_csv(os.path.join(model_dir, 'summary_statistics.csv'))
            ...
            print(f"  Mean MAPE: {results_df['MAPE'].mean():.4f} ± {results_df['MAPE'].std():.4f}")
    
    # Create final comparison table
    if all_results:
        comparison_df = pd.DataFrame(all_results).T
        comparison_df = comparison_df.round(4)
        comparison_df.to_csv(os.path.join(RESULTS_DIR, 'model_comparison_summary.csv'))
