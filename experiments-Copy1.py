...
# def build_tcn(input_length, num_filters=32, kernel_size=2, num_layers=4, output_size=1, dropout_rate=0.2):
...
#         skip = Conv1D(1, 1, padding='same')(conv)
#         skips.append(skip)
#         x = Add()([conv, x])  # residual
...
#     preds = []
#     current_input = np.array(initial_seq).reshape((1, look_back, 1))

#     for _ in range(n_steps):
#         pred = model.predict(current_input, verbose=0)[0, -1]
#         preds.append(pred)
#         pred_scalar = float(np.squeeze(pred))
#         next_input = np.array([[[pred_scalar]]])  # shape (1, 1, 1)
...
#     initial_seq = np.array(train[-look_back:])
#     preds = forecast_tcn(model, initial_seq, len(test), look_back)

#     return y_test.reshape(-1), preds.reshape(-1)
...
# lookback_5_bs_16_epochs_50
# lookback_5_bs_32_epochs_100
# lookback_11_bs_32_epochs_100
...
#     X_train, y_train = create_dataset(train_scaled, look_back)
#     extended_test = np.concatenate([train_scaled[-look_back:], test_scaled])
#     y_test = extended_test[look_back:]
...
#     # Forecast
#     initial_seq = train_scaled[-look_back:]
#     preds_scaled = forecast_tcn(model, initial_seq, len(test_scaled), look_back)
